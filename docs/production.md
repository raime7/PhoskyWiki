...
初始化只建两位缺失管理员及带 `isEditorialBoard=true` 的编委会诠释者；不建词条、视角、学派或分类。管理员使用网站登录，从“新建词条”填写正文后直接发布，沿用原有直编与修订管线。

初始化先在事务中获取并发锁，核验所有账号及编委会，再创建账号和凭据；任何失败整体回滚。重复或并发执行保留密码、角色、页面、正文及图片记录。已有管理员必须有完整且符合 better-auth 命名空间的 credential 账号。普通编者同邮箱、大小写歧义、半成账号、多个编委会、已删除编委会或同名未标记诠释者均明确失败；不能靠修改初始化参数静默提权/重设。先检查冲突原因，由账号恢复/内容管理流程处理，再运行相同命令。
