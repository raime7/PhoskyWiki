...
唯一历史编码兼容项是 `scripts/release-migrations.mjs` 列出的 13 组明确 SQL 文件哈希：初始 Windows 镜像的 CRLF 字节与已逐文件审查的 LF 字节等价，差异均在引号外。只允许已列出的 CRLF→LF 过渡，不做全局换行归一化，不接受新增别名或反向变化。初次升级前数据库账本仍须匹配旧镜像原始哈希；后续 LF 镜像允许账本保留这些准确旧哈希，绝不改写账本。新 SQL 继续严格核对字节，Git 属性固定迁移文件为 LF。安装发布入口时必须同时安装受审查的 `release-migrations.mjs`。
