...
进一步只读查询发现生产 16 条账本时间戳全部正确，0000–0014 使用 LF 哈希，0015 使用 CRLF 哈希，反映早期 Linux 初始化与后续 Windows 增量迁移。账本验证因此独立接受上述准确哈希对的任一成员；镜像历史验证继续只允许正向 CRLF→LF。新增混合账本回归确认两种镜像均可识别既有账本、输入账本不被修改、未知哈希仍拒绝。
