...
功能基线：`3fee40105d7a72d708e0fc5602d6cf3e6558ea54`（全部 #19–#28 已整合）。首轮验收提交：`4385b54407b5502898ac9ac3edb809899f85de66`，仅在功能基线上增加 #29 的五条组合浏览器回归。后续 `09f0af2d9bb5f82f980a5135bceb0d8ca829773a` 修正旧审核测试，使过期重提明确确认已读取的最新修订。上述首轮与复验的产品代码等同功能基线，测试树差异在下文记录。独立审查后的最终功能提交为 `ab83ca4eb8b4c5a3184c01f895bd394872e11b99`，增加别名往返及图谱缓存修复；其独立验收见文末。
...
#19–#29 共 11 个工单分别由独立 subagent 按 implement 技能完成，主任务整合后按 code-review 技能派出互相独立的 Standards / Spec 审查代理。比较基线由用户确认为本轮开始时的 HEAD `855285b5be39d1dc8aafeef31a6c0a77d1e08905`，最终复审终点为 `ab83ca4eb8b4c5a3184c01f895bd394872e11b99`。原始报告及复审报告均保存在 `.scratch/spec18-execution/review-{standards,spec}{,-recheck}.md`，没有覆盖初审发现。

### Standards

初审发现 1 项 P2 判断性问题（可能的 Primitive Obsession / Duplicated Code）：别名数组经不同分隔规则压成字符串，编辑与重提丢失元素边界。`ab83ca4` 引入统一可逆转换后，复审确认已解决。硬性标准违规 0，未解决 0，新增 0。

### Spec

初审发现 2 项 P2：别名往返违反同一元数据快照要求；图谱缓存导致删除、恢复或校对后仍读取旧关系。分别由 `ab83ca4` 和 `8fc4da0` 修复，复审确认均已解决；未解决 0，新增 0，未发现独立范围扩张。

最终发现计数：Standards 当前 0（原 1 项 P2 已解决）；Spec 当前 0（原 2 项 P2 已解决）。工单未自动关闭，代码未推送、发布或部署；真实 R2 契约仍因缺少专用凭据未执行。
...
`ab83ca4` 让新建、词条编辑、驳回重提和草稿共用 `formatAliasInput` / `parseAliasInput`。中英文逗号分隔元素，顿号保留为元素内容；包含逗号、引号、反斜杠或控制字符的单个别名使用带转义的引号编码。含糊引号明确报错并保留草稿。旧草稿未修改过的预填从原数组恢复，其余旧输入按旧规则迁移；新草稿记录格式版本。

在独立 `phosky_spec18_24` / 3124 上，原摘要编辑和重提两条浏览器用例先复现别名损坏，再验证修复。最终新增别名回归 **3/3 通过**，包括旧草稿、只改简介、新建引号错误与修正，以及新词条/词条编辑两种重提；原 `term-edit` 2 条与 `resubmission` 4 条也通过。HTTP 与纯转换定向测试 **15 条通过**，类型检查及 lint 通过。原兼容用例曾在冷路由跳转发生连接提前关闭，复跑 6 条全绿，未因此改产品；旧草稿测试增加恢复完成断言后，3 条新增用例完整复跑通过。完整经过见 `.scratch/spec18-execution/alias-review-fix.md`。

## 最终合并提交验收

2026-09-08，主任务在固定 `ab83ca4eb8b4c5a3184c01f895bd394872e11b99` 的独立检出上，使用专用数据库 `phosky_spec18_final`、生产服务器 3130，执行以下复验。全部迁移和种子仅针对该隔离库；未修改主开发库或生产环境。

| 检查 | 结果 |
| --- | --- |
| `pnpm typecheck` / `pnpm lint` | 均通过 |
| 默认 `pnpm build` | Next 16.3.4 Turbopack 生产构建通过，编译 8.5 秒 |
| `pnpm test`，真实 `SEARCH_CONTRACT_HOST=http://localhost:7700` | **258 通过、1 跳过**；34 文件通过、1 文件跳过，89.32 秒 |
| 服务契约 | 5 项真实 Meilisearch 通过；1 项真实 R2 因缺少专用凭据跳过 |
| 生产 Chrome 定向回归 | 别名 3 条、图谱缓存 2 条、真实 CLI 校对 2 条均有通过证据，分组经过见下文 |
| 基线至最终功能提交 `git diff --check` | 通过 |

生产浏览器运行 `alias-roundtrip.spec.ts`、`graph-cache.spec.ts`、`reconcile-links.spec.ts`，单 worker，首轮 **3 通过、4 失败**：3 条别名回归通过，后 4 条均在管理员连续登录准备阶段失败。按既有认证限流窗口分组复跑，原图谱 2 条在 3.9 秒全部通过，原校对 2 条在 10.5 秒全部通过；这些复跑没有修改产品或测试代码。未禁用生产认证限流。首轮全量 75 条的证据仍属于前述功能基线；本次对审查补丁相关 7 条进行生产复验，没有宣称最终提交重新执行了全部 75 条或单次全绿。

额外对同一生产测试服务连续发出四次有效登录请求，前三次 HTTP 200，第四次 HTTP 429，确认既有 10 秒最多 3 次的限流行为。该诊断只输出状态码，没有记录凭据或会话；最初未携带 Origin 的诊断请求被 403 拒绝，补齐同源头并隔窗后取得上述有效结果。

原始输出保存在 `.scratch/spec18-execution/final-patch-logs/`：`typecheck.log`、`lint.log`、`build.log`、`vitest.log`、`browser.log`、`browser-graph-retry.log`、`browser-reconcile-retry.log`、`auth-window.json`。复审结论与上述运行证据分别记录；后续提交仅更新这份验收文档。
