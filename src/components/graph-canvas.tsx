...
export function GraphCanvas({ data, height, rootId, onNodeClick, ariaLabel, ref }: GraphCanvasProps) {
  ...
  return (
    <div ref={containerRef} data-testid="graph-canvas" data-located={located ?? undefined} className="relative w-full overflow-hidden" style={{ height }}>
      ...
      <div className="absolute bottom-3 right-3 flex gap-1" role="group" aria-label="图谱视口">
        ...
        <button type="button" onClick={() => setCamera(fitCamera(layout, width, height, rootId))} className="rounded border bg-background px-2.5 py-1.5 text-sm">适应画布</button>
      </div>
    </div>
  );
}
