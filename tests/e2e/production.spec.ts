...
test("管理员从向导进入 JSON 导入后可打开新词条阅读编委会视角", async ({ page }) => {
  ...
  try {
    ...
    await page.getByLabel("导入 JSON").fill(JSON.stringify({ interpreters: [{ title: interpreterTitle }], terms: [{ title, aliases: ["导入别名"], content: "## 通俗解读\n浏览器导入正文" }] }));
...
test("测试导入后的异常仍清理本次词条、视角和诠释者，保留其他内容", async ({ page }) => {
  ...
  try {
    ...
    try {
      for (const input of [{ kind: "new_term", title, content: "清理测试视角" }, { kind: "new_interpreter", title: interpreterTitle }]) {
