...
import { pages, terms, links, termCategories, perspectives, interpreters, schools, schoolMembers } from "@/db/schema";
...
it("匿名发现以学派成员重排视角，并禁止共享缓存", async () => {
  ...
  expect(data.perspectives.slice(0, 3).map((row: { title: string }) => row.title)).toEqual([
    "编委会论主体性", "拉康论主体性", "弗洛伊德论主体性",
  ]);
...
it("编委会、置顶、直接或学派成员兴趣、其余热度的顺序保持不变", async () => {
  const root = (await listTerms()).find((row) => row.title === "主体性")!;
  const rows = await listPerspectivesOfTerm(root.id);
  const pinned = rows.find((p) => p.interpreterName === "福柯")!;
  const direct = rows.find((p) => p.interpreterName === "德勒兹")!;
  const school = (await listSchools()).find((row) => row.title === "精神分析")!;
  await getDb().update(perspectives).set({ pinnedAt: new Date() }).where(eq(perspectives.pageId, pinned.pageId));
  try {
    const data = await (await discover({ interpreters: [direct.interpreterId], schools: [school.id] })).json();
    expect(data.perspectives.slice(0, 5).map((row: { title: string }) => row.title)).toEqual([
      "编委会论主体性", "福柯论主体性", "拉康论主体性", "德勒兹论主体性", "弗洛伊德论主体性",
    ]);
    expect(data.perspectives[5].title).toBe("阿尔都塞论主体性");
  } finally {
    await getDb().update(perspectives).set({ pinnedAt: null }).where(eq(perspectives.pageId, pinned.pageId));
  }
});
